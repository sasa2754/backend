...
export class HomeController {
    ...
    public getCoursesInProgress = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.sub;
        if (!userId) throw new AppError('Usuário não autenticado.', 401);
        
        const result = await this.homeService.getCoursesInProgress(userId);
        res.status(200).json(result);
    } catch (error) {
        if (error instanceof AppError) {
            res.json({ message: error.message });
        }
        console.error("ERRO AO BUSCAR OS CURSOS EM PROGRESSO DA HOME:", error);
        res.status(500).json({ message: 'Erro interno no servidor.' });
    }
};
}